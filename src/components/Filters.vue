<template>
  <div class="flex h-auto w-full text-sm">
    <div class="flex">
      <button class="flex items-center justify-center border border-gray-400 rounded w-10 h-10" @click='changeOperationsMonth(0)' >
      ...
      <div class="flex flex items-center justify-center mx-2 px-2 border border-gray-400 rounded">
        <icon-base icon-name="calendar">
          <icon-calendar />
        </icon-base>
        <div class="text-center ml-2">{{currentMonthName}}</div>
      </div>
      <button class="flex items-center justify-center border border-gray-400 rounded w-10 h-10" @click='changeOperationsMonth(1)'>
    ...
    <input
      class="input-width ml-4 mr-4 border border-gray-400 rounded pl-2"
      type="text"
      placeholder="Поиск по проекту, контрагенту или счёту"
    />
    <div class="flex items-center border bg-gray-400 rounded">
      <button class="flex items-center rounded bg-white w-24 h-10">
        <icon-base class="ml-2" icon-name="filter">
          <icon-filter />
        </icon-base>
        <div class="text-center ml-1">Filter</div>
      ...
      <div class="bg-gray-400 v-full">
        <button class="rounded-full bg-gray-100 text-gray-500 h-8 w-8 mx-1 text-2xl leading-snug font-bold">+</button>
      </div>
    </div>
  </div>
</template>
...
.input-width {
  width: 400px;
  background: url("../assets/icons/search.svg") no-repeat scroll 360px 10px;
}
</style>
